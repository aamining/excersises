...
def load_profile(name, password)
  lines = []
  File.read("UserProfiles.txt").each_line {|line|
    lines << line.chomp
  }
  for i in 0...lines.length
    if lines[i] == name
      if lines[i+1] == password
        return lines[i], lines[i+1], lines[i+2].to_f
        break
      end
    end
  end
...
def save_profile(name, password)
  ...
  for i in 0...lines.length
    if lines[i] == name
      if lines[i+1] == password
        lines[i+2] = $currentUser.total_points*1.0
        break
      end
    end
  end
  File.open("UserProfiles.txt", "w+") do |f|
...
class Quiz
  ...
  def multiple_C(answer_list, correct_ans)
    a = answer_list
    a.delete(correct_ans)
    a.shuffle!
    multi  = a[0..2]
    multi << correct_ans
    return multi.shuffle!
  end
  ...
  def do_quiz(num_of_qs)
    ...
    while count < num_of_qs
      ...
      if answer == ans
        ...
        # puts "Lets make it easier".green
        # multi = multiple_C(@q, ans)
        # puts "1) #{multi[0]}"
        # puts "2) #{multi[1]}"
        # puts "3) #{multi[2]}"
        # puts "4) #{multi[3]}"
        # answer  = gets.chomp.to_i
        # correct = multi.find_index(ans)+1
        # if answer == correct
        #   points = 1
        #   puts "Yep, thats the correct".green
        # else
        #   puts "Nope the answer was #{ans}".red
          points =0
        #end
      end
      totalPoints += points
...
def login
  ...
  a = Artii::Base.new
  puts a.asciify('XquizzIT').green
  #puts artii "XquizzIT".green
  ...
  when "n"
    ...
    pw = gets.chomp.to_s
    $user = UserProfile.new(name,pw)
    File.open("UserProfiles.txt","a"){ |f|
      f << "\n#{name}"
      f << "\n#{pw}"
      f << "\n0"}
...
def menu
  ...
  when 1
    ...
    puts "Thats #{(points/num_of_q.to_f*100)}}%".yellow
    menu
  when 2
    make_quiz
    menu
